...
from sensorlib import cst816, qmi8658, battery # touch/spacial/battery sensors
...
def stage_rollSetup():
    ...
    while True:
        if asleep:
            ...
            # Not asleep, add small delay to save battery
            time.sleep(0.05)
        
        #Battery debugging
        if debugMode:
            display.write(uni_32, str(battery.rawAnalog()), 20, 100, c["primary"], c["background"])
            display.write(uni_32, str(battery.getLevel()), 20, 140, c["primary"], c["background"]) 
        ...
        # Enable sleep mode if screen is not used for 30 seconds
        if not press:
            i_loop += 1
            if i_loop >= screenTimeout / 0.05:
                i_loop = 0
                asleep = True
                display.sleep_mode(True)
                display.off()
                continue
        elif i_loop is not 0:
            i_loop = 0
        ...
        # Disabled IF requires press, and catches gyro in any direction
        #if press and (xyz[3] > shakeSensitivity or xyz[4] > shakeSensitivity or xyz[5] > shakeSensitivity):
        #print(round(xyz[3],0), round(xyz[4],0), round(xyz[5],0))
        if xyz[4] > shakeSensitivity:
            #print(xyz[3],xyz[4],xyz[5])
            global stage
            stage = "rollResult"
            break
...
def stage_rollResult():
    ...
    while True:
        ...
        # Hold press to exit roll result
        press = touch.get_touch()
        if press:
            i_press += 1
            if i_press >= 1.5 / 0.05:
